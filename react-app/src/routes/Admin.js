...
const Character = ({ character, onSave }) => {
    ...
    const handleDelete = (event) => {
        event.preventDefault();
        axios.delete(`${PLAYERS_API}/${character.name}`).then(() => {
            onSave();
        });
    };
    ...
    return (
        <div className="character">
            <input
                ...
                onChange={(event) =>
                    updateTempCharacter({ name: event.target.value })
                }
            />
            <input
                type="number"
                value={tempCharacter.initiative}
                onChange={(event) =>
                    updateTempCharacter({ initiative: event.target.value })
                }
            />
            <ColorSelector
                color={tempCharacter.color}
                changeColor={(event) =>
                    updateTempCharacter({ color: event.target.value })
                }
            />
            {hasBeenEdited && (
...
const CharacterList = () => {
    ...
    const fetchCharacters = () => {
        axios.get(PLAYERS_API).then((result) => {
    ...
    return (
        <div style={{ display: "flex" }}>
            ...
            <input
                ...
                onChange={(event) =>
                    updateCharacter({ initiative: event.target.valueAsNumber })
                }
            />
            <ColorSelector
                color={character.color}
                changeColor={(event) =>
                    updateCharacter({ color: event.target.value })
                }
            />
            {hasBeenEdited && (
                <button className="button" onClick={handleSubmit}>
